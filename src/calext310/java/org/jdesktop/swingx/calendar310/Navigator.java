 ...
 * as appropriate to Page which defines the grid.<p>
 * 
 * Also supports index-based access to the grid. 
 * 
 */
...
//     */
//    void setCalendar(LocalDateTime calendar);
//    
//    LocalDateTime getCalendar();
     ...
     * @return
     */
    LocalDateTime getStartOfPage();

    /**
     * Returns the Calendar which corresponds to the first cell of the grid of dates.
     * This may differ from <code>getStartOfPage</code> if the grid may contain dates before
     * the page period. 
     * 
     ...
     * @see #getStartOfPage()
     */
    LocalDateTime getFirstCell();
    
    /**
     * Returns the number of cells before the startOfPage.
     * 
     * @return
     */
    int getOffsetToMinimumLead();
     ...
     * @return
     */
    LocalDateTime getLead();
    
    void setLead(LocalDateTime dateTime);

    /**
     * Returns the value of the lead in terms of the cellType.
     * 
     * PENDING JW: consider extending to cell (as Point)
     * 
     * @return
     */
    int getLeadValue();
    ...
    /**
     * Returns the number of cell columns. This is also the number of cell
     * in a one-cell vertical scroll.
     * 
     * @return
     */
    int getColumnCount();
    ...
    /**
     * Returns the cell value at the given row/column in the grid, counting
     * starts at 1 for both. Accessing a zero index is allowed, subclasses
     * will define what to return for that case.
     * 
     * 
     * @param row the row index to access, 0 <= row <= getRowCount()
     * @param column the column index to access 0 <= column <= getColumnCount()
     * @return
     * 
     * @throws IllegalArgumentException if any is <0 or row > getRowCount or column > getColumnCount
     */
    LocalDateTime getCellValue(int row, int column);
...
//     * @return
//     */
//    DateTimeField getPageType();
//
//    /**
//     * @return
//     */
//    DateTimeField getCellType();
     ...
     * 
     * PENDING JW (all navigation methods) - might require to return a value?
     */
    void nextCell();
