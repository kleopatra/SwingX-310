...
import org.jdesktop.swingx.plaf.DateChooserUI;
 ...
 */
public class BasicDateChooserUI extends DateChooserUI {

    @SuppressWarnings("all")
    private static final Logger LOG = Logger.getLogger(BasicDateChooserUI.class
            .getName());
    
    protected JXDateChooser datePicker;
...
//                if (!(formatter instanceof DatePickerFormatterUIResource))  {
    ...
    protected void installKeyboardActions() {
        ...
        pickerMap.put(JXDateChooser.CANCEL_KEY, createCancelAction());
        pickerMap.put(JXDateChooser.COMMIT_KEY, createCommitAction());
        pickerMap.put(JXDateChooser.HOME_NAVIGATE_KEY, createHomeAction(false));
        pickerMap.put(JXDateChooser.HOME_COMMIT_KEY, createHomeAction(true));
        ...
        pickerInputMap.put(KeyStroke.getKeyStroke("ENTER"), JXDateChooser.COMMIT_KEY);
        pickerInputMap.put(KeyStroke.getKeyStroke("ESCAPE"), JXDateChooser.CANCEL_KEY);
        // PENDING: get from LF
        pickerInputMap.put(KeyStroke.getKeyStroke("F5"), JXDateChooser.HOME_COMMIT_KEY);
        pickerInputMap.put(KeyStroke.getKeyStroke("shift F5"), JXDateChooser.HOME_NAVIGATE_KEY);
    ...
    protected void installLinkPanelKeyboardActions() {
        ...
        map.put(JXDateChooser.HOME_COMMIT_KEY, datePicker.getActionMap().get(
                JXDateChooser.HOME_COMMIT_KEY));
        map.put(JXDateChooser.HOME_NAVIGATE_KEY, datePicker.getActionMap().get(
                JXDateChooser.HOME_NAVIGATE_KEY));
    ...
    protected void uninstallLinkPanelKeyboardActions(JComponent panel) {
        ...
        map.remove(JXDateChooser.HOME_COMMIT_KEY); 
        map.remove(JXDateChooser.HOME_NAVIGATE_KEY); 
    ...
    protected void uninstallListeners() {
        ...
        // monthView
        datePicker.getCalendarView().getSelectionModel().removeDateSelectionListener(monthViewSelectionListener);
        datePicker.getCalendarView().removeActionListener(monthViewActionListener);
        datePicker.getCalendarView().removePropertyChangeListener(propertyChangeListener);
...
//  --------------------- wiring listeners    
     ...
     * @param oldMonthView
     */
    protected void updateMonthViewListeners(JXCalendar oldMonthView) {
        ...
        datePicker.getCalendarView().addPropertyChangeListener(monthViewPropertyListener);
        datePicker.getCalendarView().addActionListener(monthViewActionListener);
    ...
    protected void updateSelectionModelListeners(DateSelectionModel oldModel) {
        ...
        datePicker.getCalendarView().getSelectionModel()
    ...
    protected JFormattedTextField createEditor() {
        ...
        // int columns = UIManagerExt.getInt("JXDateChooser.numColumns", null);
        // if (columns > 0) {
        // f.setColumns(columns);
        // }
        // that's always 0 as it comes from the resourcebundle
        // f.setColumns(UIManager.getInt("JXDateChooser.numColumns"));
        Border border = UIManager.getBorder("JXDateChooser.border");
        if (border != null) {
            f.setBorder(border);
        }
        return f;
    ...
    protected JButton createPopupButton() {
        ...
        Icon icon = UIManager.getIcon("JXDateChooser.arrowIcon");
        if (icon == null) {
...
// ---------------- Layout    
     ...
     * 
     * PENDING: there's a resource property JXDateChooser.numColumns - why 
     *   don't we use it?
     * 
...
//------------------------------- controller methods/classes 
    ...
    public Date getSelectableDate(Date date) throws PropertyVetoException {
        Date cleaned = date == null ? null :
            datePicker.getCalendarView().getSelectionModel().getNormalizedDate(date);
        ...
        if (datePicker.getCalendarView().isUnselectableDate(cleaned)) {
            throw new PropertyVetoException("date not selectable", null);
         }
        return cleaned;
    }
...
//        datePicker.getMonthView().ensureDateVisible(visibleHook);        
    ...
    protected void updateFromValueChanged(Date oldDate, Date newDate) {
        if ((newDate != null) && datePicker.getCalendarView().isUnselectableDate(newDate)) {
            revertValue(oldDate);
            return;
        }
        // the other place to interrupt the update spiral
        if (!CalendarUtils.areEqual(newDate, datePicker.getCalendarView().getSelectionDate())) {
            datePicker.getCalendarView().setSelectionDate(newDate);
        }
     ...
     *   may be null.
     */
    protected void updateFromMonthViewChanged(JXCalendar oldMonthView) {
        uninstallPopup();
        ...
        datePicker.getCalendarView().setComponentInputMapEnabled(true);
    ...
    private void updateEditorValue() {
        datePicker.getEditor().setValue(datePicker.getCalendarView().getSelectionDate());
    }
    ...
    protected void updateFromEditableChanged() {
        ...
        datePicker.getCalendarView().setEnabled(isEditable);
        datePicker.getEditor().setEditable(isEditable);
        /*
         * PatrykRy: Commit today date is not allowed if datepicker is not editable!
         */
        setActionEnabled(JXDateChooser.HOME_COMMIT_KEY, isEditable);
        // for consistency, synch navigation as well 
        setActionEnabled(JXDateChooser.HOME_NAVIGATE_KEY, isEditable);
    }
    ...
    protected void updateLinkDate() {
        datePicker.setLinkDay(datePicker.getCalendarView().getToday());
    }
    ...
    private void updateChildLocale(Locale locale) {
        if (locale != null) {
            ...
            datePicker.getCalendarView().setLocale(locale);
        }
    }
...
//------------------- methods called by installed actions
    ...
    protected void home(boolean commit) {
        if (commit) {
            Calendar cal = datePicker.getCalendarView().getPage();
            cal.setTime(datePicker.getLinkDay());
            datePicker.getCalendarView().setSelectionDate(cal.getTime());
            datePicker.getCalendarView().commitSelection();
        } else {
...
//            datePicker.requestFocusInWindow();
    ...
    /**
     * Popup component that shows a JXCalendar component along with controlling
     ...
     * editable field of the JXDateChooser.
     * 
     */
    protected class BasicDatePickerPopup extends JPopupMenu {
        ...
        public BasicDatePickerPopup() {
            ...
            add(datePicker.getCalendarView(), BorderLayout.CENTER);
...
//------------- implement Mouse/MotionListener        
        ...
        public void mouseReleased(MouseEvent ev) {
            ...
            if (_forwardReleaseEvent) {
                JXCalendar monthView = datePicker.getCalendarView();
...
//------------------ implement propertyChangeListener        
        ...
        public void propertyChange(PropertyChangeEvent e) {
            ...
            if (e.getSource() == datePicker.getCalendarView()) {
        ...
        private void datePickerPropertyChange(PropertyChangeEvent e) {
            ...
            } else if (JXDateChooser.MONTH_VIEW.equals(property)) {
                updateFromMonthViewChanged((JXCalendar) e.getOldValue());
            } else if (JXDateChooser.LINK_PANEL.equals(property)) {
                updateLinkPanel((JComponent) e.getOldValue());
            } else if (JXDateChooser.EDITOR.equals(property)) {
...
// ------------- implement actionListener (listening to monthView actionEvent)
        ...
        private void monthViewActionPerformed(ActionEvent e) {
            if (JXCalendar.CANCEL_KEY.equals(e.getActionCommand())) {
                cancel();
            } else if (JXCalendar.COMMIT_KEY.equals(e.getActionCommand())) {
                commit();
            }
        }
