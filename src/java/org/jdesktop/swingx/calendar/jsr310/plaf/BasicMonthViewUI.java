...
//        inputMap.put(KeyStroke.getKeyStroke("F1"), "selectPreviousDay");
    ...
    protected  static class RenderingHandler implements CalendarRenderingHandler {
        ...
        private FormatStringValue createDayStringValue(Locale locale) {
            DateTimeFormatter dayFormatter =  DateTimeFormatters.pattern("d", getSafeLocale(locale));
...
//                .toFormatter();
        ...
        private StringValue createMonthHeaderStringValue(Locale locale) {
            DateTimeFormatter monthFormatter = new DateTimeFormatterBuilder()
                ...
                .appendLiteral(" ")
                .appendPattern("yyyy")
                .toFormatter(getSafeLocale(locale));
        ...
        protected Locale getSafeLocale(Locale locale) {
        ...
        private StringValue createWeekOfYearStringValue(Locale locale) {
            DateTimeFormatter weekOfYearFormatter = DateTimeFormatters.pattern("D", getSafeLocale(locale));
...
//---------------------- listener creation    
    ...
    protected Point getDayGridPosition(LocalDateTime date) {
        ...
        LocalDateTime startOfDay = LocalDateTime.ofMidnight(date.toLocalDate());
    ...
    protected LocalDateTime getDayInMonth(LocalDateTime month, int row, int column) {
        ...
        Month monthOfYear = month.getMonth();
        LocalDateTime startOfWeek = DateTimeUtils.startOfWeek(month);
        LocalDateTime day = startOfWeek.plusDays((row * JXMonthView.DAYS_IN_WEEK + column));
        if (monthOfYear == day.getMonth()) {
            return day;
        } 
        return null;
    ...
    protected Point getMonthGridPosition(LocalDateTime date) {
        ...
        int firstMonth = calendar.getMonth().getValue();
...
//------------------------ layout
        ...
        public void layoutContainer(Container parent) {
            ...
            for (Month month: Month.values()) {
                calendar = DateTimeUtils.startOfMonth(calendar.withMonth(month.getValue()));
