    ...
    private HighlightPredicate getTodayPredicate() {
        HighlightPredicate predicate = new HighlightPredicate() {
            ...
            public boolean isHighlighted(Component renderer,
                ...
                if (adapter.getValue() instanceof OffsetDateTime) {
                    LocalDate date = ((OffsetDateTime) adapter.getValue()).toLocalDate();
...
//            weekDays = view.getDaysOfTheWeek();
            ...
            DayOfWeek first = DateTimeUtils.getFirstDayOfWeek(Locale.getDefault());
            DayOfWeek day = first;
            Map<DayOfWeek, String> dayTexts = DateTimeUtils.getShortDayOfWeekTexts(Locale.getDefault());
            week = new ArrayList<String>();
            for (int i = 1; i <= 7; i++) {
                week.add(dayTexts.get(day));
                day = day.plus(1);
            }
        }
        ...
        @Override
        protected boolean isValidFieldValue(int fieldValue) {
            YearMonth month = YearMonth.from(getCurrentDateTime());
//            int max = month.lengthInDays(getCurrentDateTime().isLeapYear());
            return month.isValidDay(fieldValue);
        }
    ...
    public static class MonthTableModel extends LinkTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            int fieldValue = getFieldValue(rowIndex, columnIndex);
            if (isValidFieldValue(fieldValue)) {
                return getCurrentDateTime().withMonth(fieldValue);
            }
            return null;
        }
